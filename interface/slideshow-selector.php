<div class="bu-slideshow-selector">
	...
	<div class="bu-slideshow-selector-basic">
			<p>
				<label for="bu_slideshow_selected"><?php esc_html_e('Select Slideshow:', BU_SSHOW_LOCAL); ?></label><br/>
			...
			<p>
				<label for="bu_slideshow_select_transition"><?php esc_html_e('Select transition type:', BU_SSHOW_LOCAL); ?></label><br />
				<select name="bu_slideshow_select_transition" id="bu_slideshow_select_transition">
					<?php foreach (BU_Slideshow::$transitions as $transition) {
						...
						printf('<option value="%s"%s>%s</option>', esc_attr($transition), esc_attr($sel), esc_html(ucwords($transition)));
					} ?>
				</select><br />
			</p>
			<p>
				<input type="checkbox" name="bu_slideshow_show_nav" id="bu_slideshow_show_nav" value="1"<?php echo $args['show_nav'] ? ' checked="checked"' : ''; ?>/> <label for="bu_slideshow_show_nav"><?php esc_html_e('Display navigation', BU_SSHOW_LOCAL); ?></label>
			</p>
			<p class="bu_slideshow_nav_style" <?php echo $args['show_nav'] ? ' style="display:inline;"' : ''; ?>>
				<label for="bu_slideshow_nav_style"><?php esc_html_e('Navigation style:', BU_SSHOW_LOCAL); ?></label><br />
				<select name="bu_slideshow_nav_style" id="bu_slideshow_nav_style">
					<?php foreach (BU_Slideshow::$nav_styles as $style) {
						...
						printf('<option value="%s"%s>%s</option>', esc_attr($style), esc_attr($sel), esc_html(ucwords($style)));
					} ?>
				</select>
			</p>
			<p>
				<input type="checkbox" name="bu_slideshow_autoplay" id="bu_slideshow_autoplay" value="1"<?php echo $args['autoplay'] ? ' checked="checked"' : ''; ?> /> <label for="bu_slideshow_autoplay"><?php esc_html_e('Automatically play slideshow', BU_SSHOW_LOCAL); ?></label>
			</p>
			<p>
				<input type="checkbox" name="bu_slideshow_shuffle" id="bu_slideshow_shuffle" value="true"<?php echo $args['shuffle'] ? ' checked="checked"' : ''; ?> /> <label for="bu_slideshow_shuffle"><?php esc_html_e('Shuffle slides', BU_SSHOW_LOCAL); ?></label>
			</p>
			
			<p>
				<label for="bu_slideshow_width"><?php esc_html_e('Fixed width:', BU_SSHOW_LOCAL); ?></label><br />
			...
			<fieldset id="bu_slideshow_alignment">
				<legend><?php esc_html_e('Alignment:', BU_SSHOW_LOCAL); ?></legend>
			...
			<p>
				<label for="bu_slideshow_custom_trans"><?php esc_html_e('Custom Transition:', BU_SSHOW_LOCAL); ?></label><br />
			...
			<p>
				<label for="bu_slideshow_transition_delay"><?php esc_html_e('Slide transition delay:', BU_SSHOW_LOCAL); ?></label><br />
	...
	<p><?php esc_html_e('No slideshows found.', BU_SSHOW_LOCAL); ?> <a href="<?php echo esc_url(BU_Slideshow::$add_url); ?>" title="<?php esc_attr_e('Create a new slideshow', BU_SSHOW_LOCAL); ?>"><?php esc_html_e('Add a Slideshow', BU_SSHOW_LOCAL); ?></a></p>

		...
		<?php 
			foreach ($valid_templates as $i => $t){
				if( is_array( $t ) && array_key_exists( 'name', $t ) ){
					printf( '<option value="%s" %s>%s</option>', esc_attr($i), selected( $i, esc_attr($template_id), false ), esc_html($t['name']) );
				}
			} 
		?>
	</select>
...
<p><strong><?php esc_html_e('Advanced options', BU_SSHOW_LOCAL); ?>:</strong> (<?php esc_html_e('optional', BU_SSHOW_LOCAL); ?>)<br />
	...
	<input type="number" id="bu_slideshow_height" name="bu_slideshow_height" value="<?php echo esc_attr($slide->get());$height; ?>" min="0" max="9999" />
	<span>px</span>
</p>
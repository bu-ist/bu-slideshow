				...
				<?php
					if($img_thumb){
						...
						foreach ($img_meta['sizes'] as $size => $d) {
							$selected = ($size == esc_attr($this->image_size)) ? " selected " : "";
							$display_size = $size;
		...
		<div class="bu-slide-caption-container">
			...
			<select id="bu_slides[<?php echo esc_attr($this->order); ?>][caption][position]" name="bu_slides[<?php echo esc_attr($this->order); ?>][caption][position]" >
				...
				foreach($caption_positions as $l=>$v){
					...
					printf("<option value='%s' %s>%s</option>", esc_attr($v), esc_attr($maybe_selected), esc_attr($l));
				}
				?>
			</select>
			</p>
			...
			<?php
				foreach( $allowed_fields as $id => $field ){
					...
					echo "<p>";
					printf('<label for="bu_slides[%s][custom_fields][%s]">%s</label>', esc_attr($this->order), esc_attr($id), esc_attr($field['label'] ) );

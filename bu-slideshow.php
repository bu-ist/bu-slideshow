...
class BU_Slideshow {
	...
	static public function delete_slideshow_ajax() {
		...
		echo esc_html(self::delete_slideshow($id));
		exit;
	}
	...
	*/
	...
	$msg = !empty($_GET['msg']) ? filter_var( $_GET['msg'], FILTER_SANITIZE_STRING ) : '';
		...
		$show->set_view('admin');
		echo esc_html__($show->get(array('msg' => $msg)));
	}
	...
	static public function admin_footer() {
		...
		?>
			<div id="bu_slideshow_modal_wrap" class="wrap postbox">
				...
				<?php echo esc_html__(self::get_selector()); ?>
	...
	static public function add_media_button() {
		if ( self::using_editor() ) {
			echo sprintf( '<a class="button" id="bu_slideshow_modal_button" title="%s" href="#">%s</a>', esc_html__( 'Add Slideshow', BU_SSHOW_LOCAL ), esc_html__( 'Add Slideshow', BU_SSHOW_LOCAL ) );
		}
	}
